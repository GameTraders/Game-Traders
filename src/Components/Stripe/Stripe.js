...
import './Stripe.css'
// import {onToken} from '../../ducks/userReducer'
...
onToken = token => {
    ...
    axios
      ...
      .then(res => {
       this.setState({})
        alert(`Points Purchase Successful!`);
      });
  };
  ...
  render() {
    ...
    return (
      ...
      className='stripe'
        ...
        // }}
      >
          <h3>Point Purchase</h3>
          <h4>Please enter the amount of points you would like to purchase</h4>
          <br/>
          <input
        ...
        />
        <StripeCheckout
          name="Point Purchase" //header
          image={imageUrl}
          description="1 point = $1" //subtitle - beneath header
          ...
          closed={() => this.props.toggleChange()} //fires cb when stripe is closed
        ...
        <br/>
        <br/>
        <h5 onClick={() => this.props.toggleChange()}>Close Window</h5>
